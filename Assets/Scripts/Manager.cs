...
public class Manager : MonoBehaviour
{
    ...
    public float timeframe = 15;
    public int populationSize = 100;//creates population size
    public GameObject prefab;//holds bot prefab

    public GameObject secondPrefab;//holds bot prefab

    public int[] layers = new int[3] { 5, 3, 2 };//initializing network to the right size

    [Range(0.0001f, 1f)] public float MutationChance = 0.5f;    //0.5f

    [Range(0f, 1f)] public float MutationStrength = 0.1f;       //0.1f
    ...
    public void CreateBots()
    {
        ...
        for (int i = 0; i < populationSize; i++)
        {
            Bot car = Instantiate(prefab, new Vector3(0, 1.6f, -16), prefab.transform.rotation).GetComponent<Bot>();//create botes
